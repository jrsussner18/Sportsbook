        ...
        for x, index in enumerate(read):
            ...
            if index[-1] == test.split(" ")[1]:
                ...
                spread_odds_away = Button(
                    away_frame, text=index[3], fg="red", bg="blue", command=lambda: calcOdds(index[3])
                )
                ml_away = Button(away_frame, text=index[4], fg="red", bg="blue", command=lambda: calcOdds(index[4]))
                spread_home = Label(home_frame, text=index[5], fg="blue", bg="red")
                spread_odds_home = Button(
                    home_frame, text=index[6], fg="blue", bg="red", command=lambda: calcOdds(index[6])
                )
                ml_home = Button(home_frame, text=index[7], fg="blue", bg="red", command=lambda: calcOdds(index[7]))
                # Initialize Buttons
                away.pack(side = 'left')
                widgets.append(away)
                home.pack(side = 'left')
